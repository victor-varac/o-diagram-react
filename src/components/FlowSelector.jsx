...
const FlowSelector = ({ flows, selectedFlow, selectedFolder, onSelectFlow, onSelectFolder }) => {
  return (
    <div style={{
      ...
      {/* Tab for folder selection (currently only proceso-rol, extensible) */}
      <div style={{
        display: 'flex',
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', flex: 1 }}>
          {Object.entries(flows).map(([key, flow]) => (
            <button
              ...
              style={{
                padding: '8px 16px',
                border: selectedFlow === key ? '2px solid #FF6D00' : '2px solid #e0e0e0',
                borderRadius: '8px',
                background: selectedFlow === key ? '#FFF3E0' : 'white',
                color: selectedFlow === key ? '#FF6D00' : '#666',
                cursor: 'pointer',
                fontSize: '13px',
